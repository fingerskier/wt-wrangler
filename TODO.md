...
## 2. Garbage-collect WT fragment files + name-collision guard
`writeFragmentFile` uses `${layout.window}.json` as the fragment filename inside `%LOCALAPPDATA%\Microsoft\Windows Terminal\Fragments\wt-wrangler\`. Two layouts that reuse the same `window` name silently overwrite each other's transient `wtw-<window>-<base>` profiles, and unused fragments are never cleaned. Namespace by layout-id (or content hash) and add a startup sweep that drops fragments not referenced by any layout in the current saveDir tree.
